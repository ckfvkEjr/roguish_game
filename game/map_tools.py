...
def generate_shop_items_for_room(tilemap):
    ...
    for (cx, cy) in spots:
        ...
        if choose_rd:
            etype = "rd"
            key   = random.choice(rd_keys)
            price = 3 + 2 * (config.itdiff() - 1)
        else:
            etype = "drop"
            key   = random.choice(drop_keys)
            price = 5 + 2 * (config.itdiff() - 1)

        ent = Entity(cx * TILE_SIZE, cy * TILE_SIZE, key, entity_type=etype)
        result.append({"ent": ent, "price": price})
        print("spots:", len(spots), spots, "generated:", len(result), [g["ent"].symbol for g in result])
        print("8개:", len(spots), "생성:", len(result))
    return result
